...
expr = do
  ...
  where
    ...
    psub t = do
      symbol "-"
      e <- expr
      return (t - e) -- it's right associative

term :: Parser Int 
term = do
  f <- factor
  pmul f <|> pdiv f <|> return f
  where
    pmul f = do
      symbol "*" 
      t <- term 
    ...
    pdiv f = do
      symbol "/"
      t <- term
      return (f `div` t) -- strange behavior, but let it be
...
sqroot n = snd $ head $ dropWhile (not . valid) gap
  where
    seq = iterate (\a -> (a + n / a) / 2) 1.0
    gap = zip seq (tail seq)
    valid (x, y) = abs (x - y) <= 1e-5
