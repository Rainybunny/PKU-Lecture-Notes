...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | x :: s1' =>
    ...
    | true => included s1' (remove_one x s2)
    | false => false
    end
  end.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    destruct (x =? 0) eqn:Eqx.
    + rewrite leb_n_Sn. reflexivity.
    + simpl. rewrite Eqx. apply IHs.
Qed.
(** [] *)
...
Theorem involution_injective_any : forall (X : Type) (f : X -> X),
    (forall n : X, f (f n) = n) -> (forall n1 n2 : X, f n1 = f n2 -> n1 = n2).
Proof.
  intros.
  rewrite <- (H n1).
  rewrite <- (H n2).
...
Theorem rev_injective : forall (l1 l2 : natlist),
  rev l1 = rev l2 -> l1 = l2.
Proof.
  apply (involution_injective_any natlist rev rev_involutive).
Qed.
(** [] *)
