...
Proof.
  ...
  - intros.
    ...
    reflexivity.
  - destruct x as [x y].
    destruct (split l) as [lx ly] eqn:El.
    replace (split ((x, y) :: l)) with (x :: lx, y :: ly).
    + intros.
...
Proof.
  ...
  - destruct (f false) eqn:Ffalse.
    + destruct b.
      * repeat rewrite Ftrue. reflexivity.
      * rewrite Ffalse. repeat rewrite Ftrue. reflexivity.
    + destruct b.
      * repeat rewrite Ftrue. reflexivity.
      * repeat rewrite Ffalse. reflexivity.
  - destruct (f false) eqn:Ffalse.
    + destruct b.
      * rewrite Ftrue. rewrite Ffalse. rewrite Ftrue. reflexivity.
      * rewrite Ffalse. rewrite Ftrue. rewrite Ffalse. reflexivity.
    + destruct b.
      * rewrite Ftrue. repeat rewrite Ffalse. reflexivity.
      * repeat rewrite Ffalse. reflexivity.
Qed.
...
Proof.
  intros.
  destruct (n =? m) eqn:E.
  - rewrite (eqb_true n m E).
    symmetry.
    apply eqb_refl.
  - destruct (m =? n) eqn:E'.
    + rewrite (eqb_true m n E') in E.
...
Proof.
  intros.
  assert (E : n = m). { apply eqb_true. apply H. }
  assert (E0 : m = p). { apply eqb_true. apply H0. }
  rewrite E.
  rewrite E0.
  apply eqb_refl.
Qed.
...
Fixpoint trunk {X} (l : list X) n : list X :=
  match l with
  | nil => nil
  | x :: l' =>
    match n with
...
Definition split_combine_statement : Prop :=
  ...
  combine l1 l2 = l ->
  split l = (trunk l1 (length l2), trunk l2 (length l1)).
...
Proof.
  ...
  - destruct x as [x y].
    ...
    destruct (split l) as [lx ly] eqn:El.
    simpl. rewrite El.
    destruct l1 as [ | x' l1']. { discriminate H. }
    destruct l2 as [ | y' l2']. { discriminate H. }
    simpl in H. injection H as H1 H2.
    rewrite H1. rewrite H2. simpl.
    injection (IHl l1' l2' H) as Hx Hy.
    repeat f_equal.
    + apply Hx.
    + apply Hy.
Qed.
