...
Proof.
  unfold not.
  intros.
  specialize contradiction_implies_anything with (P:=P) as C.
  specialize C with (Q:=False).
  apply C in H.
  apply H.
Qed.
...
Proof.
  ...
  apply conj.
  - intros.
    assert (HPQ : P \/ Q). { left. apply H0. }
    apply H in HPQ.
    apply HPQ.
  - intros.
    assert (HPQ : P \/ Q). { right. apply H0. }
    apply H in HPQ.
    apply HPQ.
Qed.
...
Definition disc_fn_list X (l : list X) : Prop :=
  match l with
  | nil => True
  | _ => False
  end.
...
Proof.
  unfold not.
  intros.
  assert (E : disc_fn_list X nil). { simpl. apply I. }
  rewrite H in E.
  simpl in E.
  apply E.
Qed.
...
Proof.
  ...
  - intros [ | n'] [x Hx].
    ...
    + simpl. simpl in Hx.
      injection Hx as Hx.
      specialize IHm with (n := n').
      assert (Hm : exists y, m = n' + y). { exists x. apply Hx. }
      apply IHm in Hm. apply Hm.
Qed.
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    ...
    + intros. simpl in H.
      ...
      * apply IHl' in H.
        destruct H as [u [Hfu Hiu]].
        exists u. split.
        -- apply Hfu.
        -- unfold In. right. apply Hiu.
  - intros [x [Hfx Hix]].
    rewrite <- Hfx.
    apply In_map. { apply Hix. }
Qed.
...
Proof.
  ...
  - intros. split.
    + simpl. intros.
      destruct H as [H | H].
      * left. unfold In. left. apply H.
      * apply IH in H. destruct H as [H | H].
        -- left. unfold In. right. apply H.
        -- right. apply H.
    + simpl. intros [[H | H] | H].
      * left. apply H.
      * right.
        assert (H' : In a l' \/ In a l'0). { left. apply H. }
        apply IH in H'.
        apply H'.
      * right.
        assert (H' : In a l' \/ In a l'0). { right. apply H. }
        apply IH in H'.
        apply H'.
Qed.
...
Proof.
  ...
  - simpl. split.
    + intros. split.
      * specialize H with x.
        assert (E : x = x \/ In x l). { left. reflexivity. }
        apply H in E.
        apply E.
      * assert (H' : forall y : T, In y l -> P y). {
          intros.
          specialize H with y.
          assert (H0' : x = y \/ In y l). { right. apply H0. }
          apply H in H0'.
          apply H0'.
        }
        apply IHl in H'.
...
Proof.
  ...
  - destruct (even n) eqn:En.
    + rewrite even_S.
      rewrite En. simpl.
      destruct IHn as [k Hnk].
      exists k. rewrite Hnk.
      reflexivity.
    + rewrite even_S.
      rewrite En. simpl.
      destruct IHn as [k Hnk].
      exists (S k). rewrite Hnk.
      reflexivity.
Qed.
    ...
    + simpl. split.
      * intros E.
        ...
        --specialize IHl1 with l2.
          ...
          apply El.
      * intros E.
        injection E as Ev El.
        apply H in Ev.
        specialize IHl1 with l2.
        apply IHl1 in El.
        rewrite Ev. rewrite El.
        reflexivity.
Qed.
...
Lemma rev_append_tail : forall X (l1 l2 : list X),
...
Proof.
  ...
  - reflexivity.
  - intros. simpl.
    specialize IHl1 with [x] as IHlx.
    rewrite IHlx.
    rewrite <- app_assoc.
    simpl.
    specialize IHl1 with (x :: l2) as IHll2.
    apply IHll2.
Qed.
...
Proof.
  ...
  intros.
  apply H.
  assert (T : P -> P \/ (P -> False)). { intros. left. apply H0. }
  right.
  intros HP.
  apply T in HP.
  apply H in HP.
...
Proof.
  ...
  specialize EM with (P x).
  destruct EM as [Hx | Hx].
  - apply Hx.
  - assert (nH : exists y, ~ P y). { exists x. apply Hx. }
    apply H in nH.
    destruct nH.
Qed.
...
(* proved later *)
Definition equivalent_loop : Prop :=
...
Lemma peirce'double_negation_elimination :
  peirce -> double_negation_elimination.
Proof.
  intros A.
  unfold peirce in A.
...
Lemma double_negation_elimination'de_morgan_not_and_not :
  double_negation_elimination -> de_morgan_not_and_not.
Proof.
  intros A.
  unfold double_negation_elimination in A.
  unfold de_morgan_not_and_not.
  intros.
  specialize A with (P \/ Q).
  apply A.
  specialize contrapositive with (~ (P \/ Q)) (~ P /\ ~ Q) as C.
  apply C.
  apply de_morgan_not_or.
  apply H.
Qed.

Lemma de_morgan_not_and_not'implies_to_or :
  de_morgan_not_and_not -> implies_to_or.
Proof.
  intros A.
  unfold de_morgan_not_and_not in A.
  unfold implies_to_or.
  intros. apply A. unfold not.
  intros [HP HQ]. apply HP.
  intros T. apply H in T.
  apply HQ. apply T.
Qed.

Lemma implies_to_or'excluded_middle :
  implies_to_or -> excluded_middle.
Proof.
  unfold implies_to_or.
  unfold excluded_middle.
  intros A P.
  assert (HP : P -> P). { intros H. apply H. }
  specialize A with P P.
  apply A in HP.
  destruct HP.
  - right. apply H.
  - left. apply H.
...
Lemma excluded_middle'consequentia_mirabilis :
  excluded_middle -> consequentia_mirabilis.
Proof.
  unfold excluded_middle.
  ...
  destruct A as [A | A].
  - apply A.
  - apply H in A. apply A.
Qed.

Lemma consequentia_mirabilis'peirce :
  consequentia_mirabilis -> peirce.
Proof.
  unfold consequentia_mirabilis.
  unfold peirce.
...
Theorem all_equivalent_axioms : equivalent_loop.
Proof.
  unfold equivalent_loop.
  repeat split.
  - apply peirce'double_negation_elimination.
  - apply double_negation_elimination'de_morgan_not_and_not.
  - apply de_morgan_not_and_not'implies_to_or.
  - apply implies_to_or'excluded_middle.
  - apply excluded_middle'consequentia_mirabilis.
  - apply consequentia_mirabilis'peirce.
Qed.
