...
\begin{array}{r|l}
	1 & \textit{occur}\gets\{\}\\
	2 & \dat{function}~~\text{check}(\textit{ptr}):\\
	3 & \qquad \textbf{if}~~\textit{ptr}=\textit{Nullptr}~~\textbf{then}\\
	4 & \qquad \qquad \textbf{return}~~\textit{True}\\
	5 & \qquad \textbf{if}~~\textit{ptr}\in\textit{occur}~~\textbf{then}\\
	6 & \qquad \qquad \textbf{return}~~\textit{False}\\
	7 & \qquad \textit{occur}\text{.insert}(\textit{ptr})\\
	8 & \qquad \textbf{if}~~\textit{ptr}\to\textit{tag}=\textit{ATOM}~~\textbf{then}\\
	9 & \qquad \qquad \textbf{return}~~\textit{True}\\
	10 & \qquad \textbf{return}~~\text{check}(\textit{ptr}\to\textit{un.ptr.hp})\\
	11 & \qquad \qquad ~~{}\land\text{check}(\textit{ptr}\to\textit{un.ptr.tp})
\end{array}
$$
&nbsp;
...
```cpp
void gemv(const CSR& A, const double* x, double* y) {
    int n = A.row;
    for (int i = 0; i < n; ++i) {
        y[i] = 0;
        for (int j = row_ptrs[i]; j < row_ptrs[i + 1]; ++j) {
            y[i] += A[i][col_index[j]] * x[j];
        }
    }
}
```
...
&emsp;&emsp;(1) 忽略常数因子, 设维护字符串总个数为 $n$, 总长度为 $L$, Trie 树在最坏情况 (树远未满) 时需要 $\mathcal O(|\Sigma|L)$ 的空间, 而 MPT 保证每次创建分支结点时一定对应着不同的字符串的分歧, 因此只有 $\mathcal O(n)$ 个分支节点, 总空间 $\mathcal O(n+L)$, 或写作 $\mathcal O(L)$ (与 $|\Sigma|$ 无关).

&emsp;&emsp;(2) 使用 hash 桶或者平衡树维护每个结点处的非空孩子 (而非直接创建 $|\Sigma|$ 个孩子). 这样最坏空间也能降至 $\mathcal O(L)$.