...
&emsp;&emsp;研究方程 $\ddot x+2\beta\omega\dot x+\omega^2x=0$, 它的解明显构成一个线性空间. 容易目测一个特解形式 $x=e^{rt}$, 验证
$$
r^2+2\beta\omega r+\omega^2=0\Ra r=r_1\lor r=r_2.最后得到通解 $x=c_1e^{r_1t}+c_2e^{r_2t}$.
$$
&emsp;&emsp;进一步, 如果有 $\ddot x+2\beta\omega\dot x+\omega^2x=f(t)$, 应当有 $x=c_1x_1+c_2x_2+x_3$.

&emsp;&emsp;回到简谐运动, $\ddot x+\omega^2x=0$, 方程 $r^2+\omega^2=0\Ra r=\pm i\omega$, 这给出了三角函数的特解.