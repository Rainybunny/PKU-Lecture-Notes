...
def test_ewisepow_backward():
    gradient_check(
        power,
        Tensor(np.clip(np.random.randn(5, 4), 0.1, None)),
        Tensor(2 + np.random.randn(5, 4)),
    )
...
def test_log_backward():
    gradient_check(
        log, Tensor(np.clip(np.random.randn(5, 4) + 0.1, 0.1, None))
    )

def test_exp_backward():
    gradient_check(
        exp, Tensor(np.random.randn(5, 4))
    )
