...
&emsp;&emsp;按照作业要求，这里直接复用了第一次作业的代码和报告。用 Pytorch 实现简易的 LeNet 模型。拟合性能相对较弱。我们在 Task 3 中完成的 `cifar10_torch.py` 则采用了 VGG-like 模型，达到了 $90\%$ 以上的测试集准确率。在 `torch` 等必要 module 安装就绪的前提下，直接 `python cifar.py` 即可运行。
