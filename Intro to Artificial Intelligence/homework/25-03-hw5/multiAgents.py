...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def nextLayer(originDepth, originIndex, newState):
            if originIndex + 1 == newState.getNumAgents():
                return maximizer(newState, originDepth - 1, 0)
            else:
                return minimizer(newState, originDepth, originIndex + 1)

        def functorMinimax(fn):
            def searcher(state, depth, index_of_agent):
                action = None
                if state.isWin() or state.isLose() or depth == 0:
                    return self.evaluationFunction(state), None
                best = -fn(INF, -INF)
                for act in state.getLegalActions(index_of_agent):
                    cur = nextLayer(depth, index_of_agent,
                                    state.generateSuccessor(index_of_agent, act))[0]
                    if fn(cur, best) != best:
                        best, action = cur, act
                return best, action
            return searcher

        maximizer = functorMinimax(max)
        minimizer = functorMinimax(min)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def nextLayer(originDepth, originIndex, newState, alpha, beta):
            if originIndex + 1 == newState.getNumAgents():
                return maximizer(newState, originDepth - 1, 0, alpha, beta)
            else:
                return minimizer(newState, originDepth, originIndex + 1, alpha, beta)


        def maximizer(state, depth, index_of_agent, alpha, beta):
            # print("max: ", state.state, depth, index_of_agent, int(alpha), int(beta))
            maxiAction = None
            ...
            for act in state.getLegalActions(index_of_agent):
                # print("\tgen", act)
                cur = nextLayer(depth, index_of_agent,
                                state.generateSuccessor(index_of_agent, act), alpha, beta)[0]
                if cur > value: value, maxiAction = cur, act
                if value > beta: return value, maxiAction # pruned
                alpha = max(alpha, value)

            return (value, maxiAction)
        
        def minimizer(state, depth, index_of_agent, alpha, beta):
            # print("min: ", state.state, depth, index_of_agent, int(alpha), int(beta))
            miniAction = None
            def terminal_condition(state,depth):
                return state.isWin() or state.isLose() or depth == 0
            ...
            for act in state.getLegalActions(index_of_agent):
                ...
                cur = nextLayer(depth, index_of_agent,
                                state.generateSuccessor(index_of_agent, act), alpha, beta)[0]
                if cur < value: value, miniAction = cur, act
                if value < alpha: return value, miniAction # pruned
                beta = min(beta, value)
            return (value, miniAction)
        # initialize alpha/beta
        action = maximizer(gameState, self.depth, 0, -INF, INF)[1]
        return action 
...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        class Node:
            ...
            def successors(self): # Returning available (node, action) list
                return list(filter(lambda x: x[0] != None, zip(
                    [self.north, self.east, self.west, self.south, self.stop],
        ...
        def Selection(cgs, cgstree):
            ...
            '''
            # assert (not cgs.isWin()) and (not cgs.isLose())
            while (not cgs.isWin()) and (not cgs.isLose()):
                suc, acts = cgstree.successors(), cgs.getLegalActions()
                if len(suc) != len(acts): break
                cgs, act = best_UCT(suc)
                cgstree = cgstree.__getattribute__(act.lower())
            return (cgs, cgstree)

        def Expansion(cgstree):
            state = cgstree.statevalue
            ...
            for act in legal_actions:
                leaf = Node([state.generateSuccessor(0, act), 0, 1])
                leaf.parent = cgstree
                cgstree.__setattr__(act.lower(), leaf)
        ...
        def Simulation(cgs, cgstree):
            ...
            if cgstree.statevalue.isWin(): simulation_score = 500
            else: simulation_score = -500
