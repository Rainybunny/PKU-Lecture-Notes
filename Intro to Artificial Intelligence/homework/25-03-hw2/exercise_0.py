...
def b2(scores:dict):
    ...
    '''
    ret = [name for name, _ in filter(lambda x: x[1] > 50, scores.items())]
    ret.sort()
    return ret
...
def q2(n):
    ...
    '''
    x = np.add.outer(np.arange(n), np.arange(n))
    return x
    ...
    class Student:
        ...
        def __lt__(self, oth):
            if self.score != oth.score:
                ...
                return self.name < oth.name # keep it's property as t.o.

    res = [Student(name = name, score = score) for name, score in name_score_dict.items()]
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        maz = [[ord(' ') for _ in range(self.width)] for _ in range(self.height)]
        # print(self.width, self.height, len(maz), len(maz[0]))
        # for r, c in self.walls: print(r, c)
        for r, c in self.walls: maz[r][c] = ord('%')
        for r, c in self.foods: maz[r][c] = ord('.')
        maz[self.pacman_pos[0]][self.pacman_pos[1]] = ord('P')
        ret = str()
        for s in maz:
            for c in s:
                ret += chr(c)
...
if __name__ == "__main__":
    print(Layout().build_layout("example.lay").recover_layoutText())