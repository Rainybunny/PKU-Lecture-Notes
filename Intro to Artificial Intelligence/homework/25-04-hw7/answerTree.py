...
def entropy(Y: np.ndarray):
    ...
    """
    # MYCODE
    frq = np.unique(Y, return_counts=True)[1]
    frq = frq / frq.sum()
    return -(frq * np.log2(frq)).sum()
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    for i in range(len(ufeat)):
        ret -= entropy(Y[list(filter(lambda j: feat[j] == ufeat[i], range(len(feat))))]) * featp[i]
    return ret
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int],
              depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    """
    递归构建决策树。
    ...
    # print(f"current entropy = {entropy(Y)}, current depth = {depth}")
    # MYCODE
    if depth == 0 or entropy(Y) <= hyperparams["purity_bound"]: return root

    gains = [gainfunc(X, Y, i) for i in unused]
    idx = np.argmax(gains)
    # print(f"current gain: {gains[idx]}")
    ...
    allidx = range(len(feat))
    for f in ufeat:
        ids = list(filter(lambda i: feat[i] == f, allidx))
        # print(f"found {len(ids)} out of {len(allidx)}, feat is {feat}")
        root.children[f] = buildTree(X[ids], Y[ids], unused,
                                     depth - 1, purity_bound, gainfunc,
                                     prefixstr + f" -> {f}")
    return root
