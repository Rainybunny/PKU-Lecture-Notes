...
&emsp;&emsp;(1) $R$ 显然是等价关系. 若 $xRx'\land yRy'$, 一方面若 $x=x'\land y=y'$, 自然 $x\circ y=x'\circ y'$; 另一方面, 若 $x\in I\land x'\in I$, 则 $x\circ y\in IS\sub I$ 且 $x'\circ y\in IS\sub I$; 对另一侧的 $y$ 同理. 总之 $R$ 尊重 $\circ$, 是同余关系.

&emsp;&emsp;(2) 作为集合, $S/R=\{[a]:a\in S\setminus R\}\sqcup [i]$, 其中 $i\in I$. 运算性质有 $\A [x],[y]\in (S/R)\setminus[i],~[x]\ol\circ[y]=[x\circ y]$, $\A[x]\in S/R,~[x]\ol\circ [i]=[i]\ol\circ [x]=[i]$.
...
$$
abab=aabb\Ra a^{-1}ababb^{-1}=a^{-1}aabbb^{-1}\Ra ba=ab.
$$
