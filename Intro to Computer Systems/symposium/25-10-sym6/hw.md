...
&emsp;&emsp;给出 $\texttt{iOPq}$ 的计算过程.

|  阶段   |                    $\texttt{iOPq Imm,rB}$                    |
| :-----: | :----------------------------------------------------------: |
|  取指   | $\begin{aligned}\texttt{icode:ifun}&\gets M_1[\texttt{PC}]\\\texttt{rA:rB}&\gets M_1[\texttt{PC}+1]\\\texttt{valC}&\gets M_8[\texttt{PC}+2]\\\texttt{valP}&\gets\texttt{PC}+10\end{aligned}$ |
|  译码   |             $\texttt{valB}\gets R[\texttt{rB}]$              |
|  执行   | $\begin{aligned}&\texttt{valE}\gets\texttt{valB}~\mathrm{OP}~\texttt{valC}\\&\text{Set}~\texttt{CC}\end{aligned}$ |
|  访存   |                              /                               |
|  写回   |              $R[\texttt{rB}]\gets\texttt{valE}$              |
| 更新 PC |               $\texttt{PC}\gets\texttt{valP}$                |

&nbsp;

**2.** (P329 4.57 A)

&emsp;&emsp;(或许有指令后缀 $\texttt{L}$ 和 $\texttt{Q}$ 的差异.) 只需要在原来 `load_use_harzard` 的基础上排除掉 "在 M 阶段才需要 `E_dstM` 寄存器值" 的情况.

```rust
load_use_harzard = (E_icode in { IMRMOVQ, IPOPQ }) && (E_dstM == d_srcB || (E_dstM == d_srcA && !(D_icode in { IRMMOVQ, IPUSHQ })))
```
